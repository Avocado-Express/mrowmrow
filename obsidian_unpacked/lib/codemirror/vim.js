    ...
    var operators = {
      change: function(cm, args, ranges) {
        ...
        if (!vim.visualMode) {
		  ...
		  text = cm.getDeletedText() ?? text;
          finalHead = cm.mapPos(anchor, -1);
		  ...
		  text = cm.getDeletedText() ?? text;
          finalHead = cm.mapPos(cursorMin(ranges[0].head, ranges[0].anchor), -1);
        }
      ...
      'delete': function(cm, args, ranges) {
        ...
        if (!vim.visualBlock) {
          ...
          finalHead = cm.mapPos(anchor, -1);
          if (args.linewise) {
            finalHead = motions.moveToFirstNonWhiteSpaceCharacter(cm, finalHead);
          }
        } else {
          text = cm.getSelection();
          var replacement = fillArray('', ranges.length);
