...
function loadApp(asarPath) {
	...
	try {
		...
		log('Failed to load app', asarPath, e);
		return false;
	}
...
if (updatedAsarPath) {
	...
	if (success) {
		...
		log('Loaded updated app', updatedAsarPath);
	} else {
		electron.dialog.showErrorBox(
			'Obsidian could not load the latest update',
			`Obsidian ${updatedAsarVersion} could not be loaded. Loading Obsidian ${currentBaseVersion} instead.\n\n` +
			`To fix this, please download and reinstall the latest version of Obsidian from https://obsidian.md/download`
		);
	}
}
